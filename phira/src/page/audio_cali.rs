...
pub struct AudioCalibrationPage {
    _audio: AudioManager,
    cali: Music,
    cali_hit: Sfx,
...
impl Page for AudioCalibrationPage {
    ...
    fn touch(&mut self, touch: &Touch, s: &mut SharedState) -> Result<bool> {
        ...
        let mut offset = config.audio_offset * 1000.;
        if self.slider.touch(touch, t, &mut offset).is_some() {
            config.audio_offset = offset / 1000.;
            return Ok(true);
        }
    ...
    fn render(&mut self, ui: &mut Ui, s: &mut SharedState) -> Result<()> {
        ...
        s.render_fader(ui, |ui| {
            ...
            let mut t = self.tm.now() as f32 - config.audio_offset;
            if t < 0. {
                t += 2.;
            }
            if t >= 2. {
            ...
            let offset = config.audio_offset * 1000.;
            self.slider
